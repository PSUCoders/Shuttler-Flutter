	...
	MapState() {
		...
		_driversSubscription = OnlineDB().driversStream().listen((drivers) {
			...
			drivers.forEach((newDriver) {
				Driver oldDriver;
				oldDriver =
				_drivers == null ? null : _drivers.firstWhere((driver) => driver.id ==
						newDriver.id, orElse: () => null);
				if (oldDriver != null) {
					if (newDriver.latLng.longitude - oldDriver.latLng.longitude == 0) {
						...
						newDriver.direction = atan(
								(newDriver.latLng.longitude - oldDriver.latLng.longitude) /
